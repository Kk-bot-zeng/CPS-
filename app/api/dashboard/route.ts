...
export async function GET(request: Request) {
  ...
  const rpc = await auth.admin.rpc("dashboard_summary", {
    p_start: start || null,
    p_end: end || null,
  ...
  for (let from = 0; ; from += pageSize) {
    let query = auth.admin
      .from("orders")
      .select(
        "payable_amount,quantity,order_status,talent_name_raw,product_name_raw,paid_at",
      )
      .order("paid_at")
  ...
  const talentMap = new Map<
    string,
    { gmv: number; gsv: number; orders: number }
  >();
  const productMap = new Map<
    string,
    { gmv: number; qty: number; talents: Set<string> }
  >();
  const dailyMap = new Map<string, { gmv: number; gsv: number }>();
  let gmv = 0,
    gsv = 0,
    quantity = 0,
    validOrders = 0;
  for (const o of all) {
    ...
    const t = talentMap.get(o.talent_name_raw) || { gmv: 0, gsv: 0, orders: 0 };
    t.gmv += amount;
    t.gsv += valid ? amount : 0;
    t.orders++;
    talentMap.set(o.talent_name_raw, t);
    const pName = o.product_name_raw || "待映射商品";
    ...
    p.gmv += amount;
    p.qty += o.quantity || 0;
    p.talents.add(o.talent_name_raw);
    productMap.set(pName, p);
    const day = o.paid_at.slice(0, 10);
    const d = dailyMap.get(day) || { gmv: 0, gsv: 0 };
    d.gmv += amount;
    d.gsv += valid ? amount : 0;
    dailyMap.set(day, d);
  }
