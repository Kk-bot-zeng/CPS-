...
export async function GET(request: Request) {
  ...
  const channel = url.searchParams.get("channel") || "all", model = url.searchParams.get("model") || "all";
  ...
  if (channel !== "all") q = q.eq("platform", channel); if (talentFilters.length) q = q.in("talent_name_raw", talentFilters); if (model !== "all") q = q.eq("model_name", model);
