...
export async function loadWarnings(channel:ChannelFilter="all", signal?:AbortSignal){const r=await fetch(`/api/sales-warnings?channel=${channel}`,{cache:"no-store",signal});const j=await r.json().catch(()=>null);if(!r.ok)throw new Error(j?.error||"预警读取失败");return j as WarningPayload;}
export async function acknowledgeWarnings(keys:string[], signal?:AbortSignal){if(!keys.length)return;const r=await fetch("/api/sales-warnings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({keys}),signal});const j=await r.json().catch(()=>null);if(!r.ok)throw new Error(j?.error||"预警确认失败");return j;}
...
export default function SalesWarningPage({channel,onRead}:{channel:ChannelFilter;onRead?:()=>void}){
 const [data,setData]=useState<WarningPayload|null>(null),[q,setQ]=useState(""),[level,setLevel]=useState("all"),[selected,setSelected]=useState<WarningRow|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState("");
 const requestSequence=useRef(0), onReadRef=useRef(onRead), selectedCloseRef=useRef<HTMLButtonElement>(null), selectedPreviousFocus=useRef<HTMLElement|null>(null);
 ...
 useEffect(()=>{
   ...
   loadWarnings(channel,controller.signal).then(async x=>{
     if(!active||sequence!==requestSequence.current)return;
     setData(x);
     const keys=x.rows.filter(r=>r.unread).map(r=>r.resource_key);
     if(keys.length) await acknowledgeWarnings(keys,controller.signal);
     if(!active||sequence!==requestSequence.current)return;
     setData({...x,unreadCount:0,rows:x.rows.map(r=>({...r,unread:false}))});
     onReadRef.current?.();
   }).catch(reason=>{
     if(!active||reason?.name==="AbortError")return;
     setError(reason instanceof Error?reason.message:"预警读取失败，请稍后重试");
 ...
 {error&&<div className="warning-error" role="alert">{error}</div>}
