          ...
          <span className="cw-field-label">选择产品或系列 <i>*</i></span>
           ...
           {pickerOpen && <div id={productMenuId} className="cw-product-menu" role="listbox" aria-label="选择产品或系列">
             ...
             <div className="cw-product-view-tabs" role="tablist" aria-label="产品选择方式">
               ...
               <button type="button" role="tab" aria-selected={pickerView === "products"} className={pickerView === "products" ? "active" : ""} onClick={() => setPickerView("products")}>按型号 <span>{filteredProducts.length}</span></button>
             </div>
             <div className="cw-product-options">
               {pickerView === "series" ? <div className="cw-product-list">{filteredSeries.length ? filteredSeries.map((series) => { const selected = selectedSeriesKeys.includes(series.key); return <button type="button" role="option" aria-selected={selected} key={`series-${series.key}`} className={selected ? "selected" : ""} onClick={() => toggleSeries(series)}><span className="cw-option-copy"><b>{series.name}</b><small>{series.products.length} 个启用型号</small></span><span className="cw-option-check" aria-hidden="true">{selected && <Check size={14} />}</span></button>; }) : <div className="cw-product-empty" role="status">没有匹配的产品系列</div>}</div>
               : <div className="cw-product-list">{filteredProducts.length ? filteredProducts.map((product) => { const selected = selectedIds.includes(product.id); return <button type="button" role="option" aria-selected={selected} key={product.id} className={selected ? "selected" : ""} onClick={() => toggleProduct(product.id)}><span className="cw-option-copy"><b>{product.promotionName || "未填写推广名"}</b><small>{product.model}{deriveProductSeries(product) ? ` · ${deriveProductSeries(product)}` : ""}</small></span><span className="cw-option-check" aria-hidden="true">{selected && <Check size={14} />}</span></button>; }) : <div className="cw-product-empty" role="status">没有匹配的具体型号</div>}</div>}
             </div>
             <div className="cw-product-menu-footer"><span>已选 {selectedSeries.length} 个系列、{selectedStandaloneProducts.length} 个单品</span><button type="button" onClick={() => closeProductPicker(true)}>完成</button></div>
           </div>}
        </div>
